...
fn add_benchmark(c: &mut Criterion) {
    {
        let mut group = c.benchmark_group("arrow_avro_read");
    ...
    {
        let mut group = c.benchmark_group("mz_avro_read");
        ...
        for log2_size in (10..=20).step_by(2) {
            ...
            group.bench_with_input(BenchmarkId::new("utf8", log2_size), &buffer, |b, buffer| {
                b.iter(|| read_mz_avro(buffer, size).unwrap())
            });
        }
    }

    {
        ...
        for log2_size in (10..=20).step_by(2) {
            ...
            group.bench_with_input(BenchmarkId::new("utf8", log2_size), &buffer, |b, buffer| {
                b.iter(|| read_avro(buffer, size).unwrap())
            });
        }
    }
}
